...
#     main()
def build_dataframe(pdf_file: Path, ref_rows, intext_rows=None):
    """
    ref_rows: list of {"reference": str, "doi": str}
    intext_rows: list of {"intext": str, "matched_reference": str, "doi": str}
    """
    records = []

    # References section DOIs
    for r in ref_rows:
        if r["doi"]:
            records.append({
                "article_id": pdf_file.stem,
                "section": "References",
                "doi": r["https://doi.org/"+"doi"]
            })

    # In-text citation DOIs (optional, if you ran the in-text matching)
    if intext_rows:
        for r in intext_rows:
            if r["doi"]:
                records.append({
                    "article_id": pdf_file.stem,
...
if __name__ == "__main__":
    pdf_file = Path("10.1002_2017jc013030.pdf")

    # Step 1: Extract DOIs from References
    ref_rows = extract_reference_dois_from_pdf(pdf_file, lookup_missing=True)

    # Step 2: (Optional) Extract in-text citations & match to refs
    # full_text = extract_pdf_text(pdf_file)
    # intext_citations = extract_intext_citations(full_text)
    # intext_rows = match_intext_to_references(intext_citations, ref_rows)
    intext_rows = None  # comment out if you want in-text support

    # Step 3: Build DataFrame
    df = build_dataframe(pdf_file, ref_rows, intext_rows)

    # Step 4: Save / display
    #print(df)
    df.to_csv("dois_extracted.csv", index=False)